...
BYTE HTTPNeedsAuth(BYTE* cFile)
{
	...
	// If you want to protect the mpfsupload page with a username and password,
	// this can be done changing the state of the flag "uploadPsw" inside the FlyportTask()
	#if defined (MPFS_USE_SPI_FLASH)
	if (uploadPsw == TRUE)
	{
		if(memcmp(cFile, (void*)"mpfsupload",10) == 0)
			return 0x00; // Authentication will be needed later
	}
	#endif
